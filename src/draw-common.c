...
#define SHADOW_LUT_X_LEN TWIN_SHADOW_EXTENT_X
#define SHADOW_LUT_Y_LEN TWIN_SHADOW_EXTENT_Y
 ...
 * - CONFIG_SHADOW_BLUR : Controls how long the Gaussian falloff extends after
 *   the configured offset.
 * - CONFIG_SHADOW_FADE_TAIL : Enforces a minimum transparent tail so the mask
 *   still feathers out cleanly at the far edge.
 *
 * Limitations:
...
void twin_shadow_border(twin_pixmap_t *shadow, twin_argb32_t color)
{
    ...
    const twin_coord_t fade_tail = TWIN_SHADOW_FADE_EXTENT;

    if (!base_alpha)
        return;

    /* Title-style windows leave the frame untouched by the shadow strip. */
    switch (shadow->window->style) {
    case TwinWindowApplication:
        /* Start below the title bar and apply the configured vertical offset.
         */
        y_start = TWIN_TITLE_HEIGHT + y_offset;
        break;
    ...
    /* Shadow mask rendering assumes ARGB32 pixel layout throughout. */
    if (shadow->format != TWIN_ARGB32)
        return;
    ...
    if (right_extent > 0) {
        for (twin_coord_t y = 0; y < win_height; y++) {
            ...
            memset(clear.argb32, 0,
                   (size_t) right_extent * sizeof(*clear.argb32));
        }
    }
    if (bottom_extent > 0) {
    ...
    /* Clamp rendering extent to the LUT dimensions. */
...
#error "Drop shadow offsets must be positive."
    ...
    /* Bottom edge strip (extends to win_width; corner adjoins at win_width). */
    if (bottom_extent > 0) {
        ...
        twin_coord_t bottom_width = win_width - left_skip;
        if (bottom_width < 0)
            bottom_width = 0;
        ...
        if (bottom_width > 0) {
            if (bottom_width > shadow_bottom_cache_capacity) {
                twin_a8_t *newbuf =
                    twin_realloc(shadow_bottom_cache_weights, bottom_width);
                if (newbuf) {
            ...
            if (!alpha_bottom)
                alpha_bottom = twin_malloc(bottom_width);
