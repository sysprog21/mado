...
#define _twin_demospline_pixmap(demospline) \
    ((demospline)->widget.window->pixmap)
...
void _twin_demospline_paint(twin_demospline_t *demospline)
{
    ...
    twin_path_curve(path, demospline->points[1].x, demospline->points[1].y,
...
static twin_dispatch_result_t _twin_demospline_update_pos(
    twin_demospline_t *demospline,
    twin_event_t *event)
{
    if (demospline->which < 0)
        return TwinDispatchContinue;
    demospline->points[demospline->which].x =
        twin_int_to_fixed(event->u.pointer.x);
    demospline->points[demospline->which].y =
...
static int _twin_demospline_hit(twin_demospline_t *demospline,
                                twin_fixed_t x,
                                twin_fixed_t y)
{
    int i;
    ...
    for (i = 0; i < NPT; i++)
        if (twin_fixed_abs(x - demospline->points[i].x) <
                demospline->line_width / 2 &&
            twin_fixed_abs(y - demospline->points[i].y) <
                demospline->line_width / 2)
            return i;
    return -1;
}
                                                 ...
                                                 twin_event_t *event)
{
    ...
    case TwinEventButtonDown:
        demospline->which = _twin_demospline_hit(
            demospline, twin_int_to_fixed(event->u.pointer.x),
            twin_int_to_fixed(event->u.pointer.y));
